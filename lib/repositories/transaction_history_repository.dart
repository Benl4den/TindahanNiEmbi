...
class TransactionHistoryRepository {
  ...
  Future<List<TransactionHistoryEntry>> recent({
    ...
    ) WHERE (?='ALL' OR type=?) AND instr(lower(title),lower(?))>0
      AND (?=1 OR type NOT IN ('GCASH_SERVICE','MAYA_SERVICE'))${day == null ? '' : ' AND occurred>=? AND occurred<?'} ORDER BY occurred DESC,type,id DESC LIMIT ?''',
      [
        type,
        type,
        search,
        includeWalletServices ? 1 : 0,
